...
function main() {
    ...
    var width = 600, height = 600; // sizes of the svg component
    var margins = {top: 20, right: 10, bottom: 50, left: 10};

    var xVals = d3.range(-1.5, 2, dx); // x
    ...
    // add the main component and set it size
    ...
    svg.on("mousemove", function(event) {
        var pt = d3.pointer(event, svg.node()); // point in the svg component coordinates
    ...
    var drag = d3.drag()
      .on("start", function(event) {
          dragStart = d3.pointer(event, svg.node())[0];
          dragEnd = dragStart;
      })
